<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="TableList.aspx.cs" Inherits="App.Web.Doc.TableList" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="header relative">
        <h1 class="pagetitle">
            列表</h1>
        <a href="/Doc/List.aspx" class="btn abs">列表视图</a>
    </div>
    <div id="search_div">
        <fieldset class="form relative moreConditions">
            <legend>查询条件</legend>
            <div class="control_group">
                <input type="hidden" class="attrid" value="0" />
                <input type="hidden" class="attrtype" value="creator" />
                <label class="control_label">
                    创建者</label>
                <div class="controls">
                    <a href="#" class="btn multiperson" title="选择人员">选择</a>
                </div>
            </div>
            ...
            <a href="javascript:void(0);" class="btn btn_primary searchBtn abs search">查询</a>
            <a href="javascript:void(0);" class="checkMore abs" title="展开查看更多查询条件">更多查询条件</a>
        </fieldset>
    </div>
    <div class="list">
        <p>
    ...
    <script type="text/javascript">
        ...
        function loadMore(pageIndex, docid) {
            $.ajax({
                ...
                data: { pageIndex: pageIndex, docid: docid, userid: 0 },
                dataType: "json",
                    ...
                    if (res != null) {
                        $.each(res, function (i, item) {
                            if (docName == "") {
                                ...
                                $(".pagetitle").text(docName + "列表");
                            }
                            if (!hasLoadColumnName) {
                                var $thead = $(".list table thead tr");
                                $("<th>").addClass("op").html("序号").appendTo($thead);
                                $.each(item.Document.Attrs, function (trindex, tritem) {
                                    $("<th>").html(tritem.AttrName).appendTo($thead);
                                });
                                $("<th>").html("创建者").appendTo($thead);
                                $("<th>").addClass("op").html("详情").appendTo($thead);
                                hasLoadColumnName = true;
                            }
                            ...
                            $.each(item.Document.Attrs, function (j, jitem) {
                                $("<td>").html(jitem.TranValue).appendTo($tr);
                            });
                            $("<td>").html(item.Creator.Name).appendTo($tr);
        ...
        function searchMore(pageIndex, docid) {
            $.ajax({
                ...
                data: { pageIndex: pageIndex, docid: docid, userid: 0, attrIds: searchAttrIds, attrVals: searchAttrVals },
                dataType: "json",
                success: function (res) {
                    if (res.length>0) {
                        $.each(res, function (i, item) {
        ...
        //get total
        function getTotal(attrIds, attrVals,docid) {
            $.ajax({
                type: "POST",
        ...
        $(document).ready(function () {
            ...
            $(".header a").attr({ "href": "/Doc/List.aspx?ID=" + docid });
            uid = $("#userid").val();
            $.ajax({
                type: "post",
            ...
            getTotal(searchAttrIds, searchAttrVals,docid);
                ...
                $("#search_div .control_group").each(function (i, item) {
                    ...
                    switch (attrtype.toString()) {
                        ...
                        case "creator":
                            ...
                            if ($(item).find(".userspan").length > 0) {
                                searchAttrIds += arrtid + "$";
                                searchAttrVals += $(item).find(".userspan").eq(0).find("input[type=hidden]").val() + "$";
                            }
                            break;
                    }
                });

<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Transfer.aspx.cs" Inherits="App.Web.Doc.Transfer" %>
...
<div class="header">
    ...
    <div class="list">
        ...
        <div class="itemtemp hidden">
            ...
            <input type="checkbox" title="选中该文档，准备移交该文档" class="tranDocSel" />
            <input type="hidden" />
            <ul class="docinfo">
    ...
    <a href="#" id="tranBtn" title="从列表中选中文档，点击此按钮移交选中的文档" class="btn btn_primary">移交</a>
    ...
    <script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res != null) {
                        ...
                        $(".more").html("没有更多" + docName).unbind("click");
                    }
                }
            });
        };
        ...
        $(document).ready(function () {
            ...
            /*********************************************************methods of transfer**************************************/
            //check box event in each doc item
            var sel_ItemCount = 0;
            $("body").on("click", ".tranDocSel", function (e) {
                var $parent = $(this).parents(".listitem");
                //console.log($(this).prop("checked"));
                $parent.toggleClass("item_selected");
                if ($(this).prop("checked"))
                    sel_ItemCount++;
                else
                    sel_ItemCount--;
                //console.log(sel_ItemCount);
                $("#tranBtn").text("移交(" + sel_ItemCount + ")");
            });
            //ready to transfer docs
            $("#tranBtn").click(function (e) {
                e.preventDefault();
                if (sel_ItemCount == 0) {
                    InfoTip.showMessage("您还没有选中任何文档", "warning", 6000);
                    return false;
                }
                var docids = "";
                var docnames = "确定将" + $(".list .listitem").eq(0).find(".docitem").eq(0).find("label").text();
                //console.log(docnames);
                $(".list .listitem").each(function (i, item) {
                    var $cb = $(item).find("input[type=checkbox]");
                    if ($cb.prop("checked")) {
                        docids += $(item).find("input[type=hidden]").val() + ",";
                        docnames += $(item).find(".docitem").eq(0).find("a").text() + ",";
                    }
                });
                //console.log(docids + docnames);
                docids = trimLastComma(docids);
                docnames = trimLastComma(docnames) + "的" + docName + "移交给如下选择的人员";
                var $div = $("#multiperson_div");
                $div.find(".sel_type").val("s");
                $div.find("p.info").remove();
                $("<p>").addClass("info").html(docnames).insertBefore($div.find(".depts_users"));
                $div.dialog({
                    title: "选择人员，将选中文档移交给该人",
                    modal: true,
                    width: 800,
                    minHeight: 300,
                    buttons: {
                        "确认移交": function () {
                            var $seluser = $div.find("#selected_users .sel_users .useritem");
                            //console.log($seluser.length);
                            if ($seluser.length > 0) {
                                $.ajax({
                                    type: "POST",
                                    url: "/handlers/doc/transferdocs.ashx",
                                    data: { docids: docids, receiverid: $seluser.eq(0).find("input[type=hidden]").val() },
                                    success: function (res) {
                                        if (res == "1") {
                                            InfoTip.showMessage("移交文档成功", "success", 6000);
                                            $div.dialog("close");
                                            $(".list .listitem").each(function (i, item) {
                                                var $cb = $(item).find("input[type=checkbox]");
                                                if ($cb.prop("checked")) {
                                            ...
                                            InfoTip.showMessage("移交文档失败", "error", 6000);
                                        }
                                    }
                                });
                            } else {
                                InfoTip.showMessage("您还没有选择任何接收文档的人员", "warning", 6000);
                            }
                        },

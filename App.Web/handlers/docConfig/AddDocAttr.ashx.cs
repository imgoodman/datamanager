...
namespace App.Web.handlers.docConfig
{
    ...
    public class AddDocAttr : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            int id = dcs.AddDocAttr(new Document { ID = docid, Attrs = new List<DocumentAttr>() { new DocumentAttr { AttrName = attrname, AttrType = dcs.getAttrType(attrtype), Description = desc, Creator = new User { ID = userid }, IsRequired = isrequired, IsSearch = issearch, IsRepeat=isrepeat, VerticalOrder = order, HorizontalOrder=0 } } });

...
{
    public class DBInventoryMng
    {
        BaseClass bc = new BaseClass();
        SysCommon SC = new SysCommon();
        private string TabelName = "InventoryBasic_Tab";
        private string InventoryDocTab = "InventoryDocs_Tab";
        ...
        /// <param name="orderstr"></param>
        /// <returns></returns>
        public List<Inventory> GetAllBasicInventory(string sqlwhere, string orderstr)
        {
            List<Inventory> dlist = new List<Inventory>();
            string sql = SC.GetAllDataStr(TabelName, "ID", sqlwhere, orderstr);
            DataSet ds = BaseClass.mydataset(sql);
            ...
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                    dlist.Add(GetInventoryBasicByID(int.Parse(dr["ID"].ToString())));
                return dlist;
            }
            else
                return null;
